#!/usr/bin/env bash
# Stop DJ FLAC Tagger: PID from start.sh, then anything still listening on port 5123.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
cd "$ROOT"

if [ -f .dj-flac-tagger.pid ]; then
...
echo "DJ FLAC Tagger stopped (port 5123 should be free)."
